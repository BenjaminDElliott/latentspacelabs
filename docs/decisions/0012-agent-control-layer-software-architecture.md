...
supersedes:
superseded_by:
revisit_trigger: Revisit when the first vertical slice (skill framework + one dispatch skill + thin CLI harness) is running end-to-end in the pilot, when a second caller (scheduler, webhook, a second agent runner, or a second agent harness beyond Claude Code) needs to invoke the ACL's skills, when the run-recorder needs to answer queries the repo-committed run reports cannot answer, or when a component defined here is found to be in the wrong place (e.g. the dispatcher needs state the library boundary cannot give it, or the skill runner needs to grow coordination the pilot did not plan for).
---
...
The ACL is exactly nine components: three that make up the skill framework itself, and six that are shared building blocks the framework composes into skills. Each has a single responsibility, a stated input/output contract, and a stated dependency direction. Nothing else is in the ACL for the pilot.
...
9. **Operator interface (thin CLI harness).**
   - Responsibility: the human (or Perplexity shell-call) entry point. For the pilot, this is a CLI whose job is argument parsing and wiring: take a skill name and arguments, hand them to the skill runner, surface the runner's structured result to the shell. Commands map 1:1 to skills: `acl run <skill-name> [--flag value …]` (generic), with a small set of convenience aliases where they are clearer (`acl dispatch LAT-<n> --approve` → `acl run dispatch-ticket --linear-issue-id LAT-<n> --approve`). Exposes `acl skills list` (via the registry) and `acl skills show <name>` (skill metadata, including `derived_from:`) so an operator can inspect what is available. Emits exit codes that reflect the skill runner's structured status (`0` ready/succeeded, `1` caution/failed, `2` blocked/stopped, `3` needs_human) so shell callers can script around it.
   - Does not: contain dispatch logic, policy rules, Linear/agent/GitHub calls, or skill-specific behaviour. A skill added to the registry is invocable via `acl run` without CLI changes; only named convenience aliases require a CLI edit. This is the structural expression of "CLI is a thin harness, not the core abstraction."
   - Depends on: the skill runner and the skill registry. It does not reach into the adapter layer except for read-only inspection commands where doing so is simpler than going through a skill (e.g. `acl skills list`).
...
- The policy evaluator is a leaf function. It never calls anything in the ACL.
...
- Load, register, or execute a skill that claims ACL-Routed authority.

The following are explicitly *not* the ACL's responsibility, and the architecture must keep them out:

- **Perplexity.** Cognition, triage, drafting, read-heavy analysis. May call the ACL via the skill runner (through the CLI harness as a shell-call, or later a dedicated Perplexity shell-call harness) but never bypasses it for ACL-Routed actions (ADR-0008 rule matrix).
...
- **GitHub.** Durable source of truth for code, docs, ADRs, run reports (during the pilot), and PRs (ADR-0001). The ACL does not manage GitHub directly; opening PRs and pushing commits is the agent runner's job (ADR-0007). A skill records the resulting PR URL via `correlation.pr_url` in the run report.
...
5. **Shared components used by the first slice skill:**
   ...
   - **Policy evaluator.** Implements `ready | caution | blocked | stop` over `## Sequencing` + blocker statuses + the autonomy-level default (L3-with-approval for ACL-dispatched agents, per ADR-0008) + the runaway-cost stop rule. No cost-band quantification yet (ADR-0008 open question 3); treat any unknown cost as `caution` rather than `ready`.
...
6. **Operator interface (thin CLI harness).** `acl run dispatch-ticket --linear-issue-id LAT-<n> --approve` (and `--dry-run` for policy-only). The convenience alias `acl dispatch LAT-<n> --approve` is acceptable as sugar, but it is wired to the same skill. `acl skills list` and `acl skills show dispatch-ticket` work from day one. Exit codes as defined above. No TUI, no config file beyond credentials.
...
A recurring question during implementation will be "should the ACL also do X?" The default answer, per ADR-0008 and this ADR, is **no** unless X is one of the nine components above or the first slice requires it. Specifically:
...
- The CLI harness being the only operator surface means Perplexity calls the ACL via shell execution, with all the brittleness of shell quoting and exit-code parsing that implies. If this becomes the dominant failure mode, that is the trigger to add a dedicated Perplexity shell-call harness (a different harness, still over the same skill runner), not to merge logic into the CLI.
...
1. Where exactly in the repo the ACL library, the skill definitions, and the CLI harness live (likely `skills/` or `.claude/skills/` for skill files per ADR-0004 open question 1, plus an `acl/` package for the framework and shared components; ties into ADR-0004 and ADR-0008 open question 1). Resolved when LAT-20 picks the runtime.
2. Whether the skill registry loads from the same directory ADR-0004 governs (`.claude/skills/`) or from a parallel ACL-owned location. Leaning same directory, so ADR-0004's `derived_from:` regime has one home. Confirm when the first skill file is written.
...
- The skill registry loads `dispatch-ticket@0.1.0`, rejects a deliberately malformed sibling skill with a readable error, and lists both states via `acl skills list`.
- A single CLI invocation of `dispatch-ticket` (directly or via the `acl dispatch` alias) dispatches one `LAT-*` ticket end-to-end and produces one run report that matches ADR-0006 and one Linear write-back that matches ADR-0003 / ADR-0006.
