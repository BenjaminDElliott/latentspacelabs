...
- **Approval and autonomy level.** The ADR-0008 autonomy level at which this skill may run (`L1-read-only`, `L2-propose`, `L3-with-approval`, `L4-autonomous`, per LAT-36 reconciliation) and whether an explicit per-invocation approval flag is required. The skill runner refuses to execute a skill above its allowed autonomy level without the approval flag — this is the structural human gate, enforced in the runner, not in the CLI.
