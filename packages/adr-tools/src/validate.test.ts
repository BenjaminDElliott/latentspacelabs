...
test('duplicate filename prefix fails', async () => {
  ...
  try {
    await writeFile(join(dir, '0001-first.md'), validAdr('ADR-0001', 'First'));
    await writeFile(join(dir, '0001-other.md'), validAdr('ADR-0001', 'Other'));
    const result = await validateAdrDirectory(dir);
    const dupErr = result.errors.find((e) => e.message.includes('duplicate filename prefix'));
...
test('duplicate frontmatter id fails', async () => {
  ...
  try {
    ...
    const result = await validateAdrDirectory(dir);
    const dupErr = result.errors.find((e) => e.message.includes('duplicate frontmatter id'));
...
test('filename prefix vs frontmatter id mismatch fails', async () => {
  ...
  try {
    ...
    const result = await validateAdrDirectory(dir);
    const err = result.errors.find((e) => e.message.includes('does not match frontmatter id'));
...
test('nested subdirectories are ignored', async () => {
  ...
  try {
    ...
    await writeFile(join(dir, 'drafts', '0099-draft.md'), validAdr('ADR-0099', 'Draft'));
    await writeFile(join(dir, '0001-first.md'), validAdr('ADR-0001', 'First'));
    const result = await validateAdrDirectory(dir);
    assert.deepEqual(result.errors, []);
    assert.deepEqual(result.filesChecked, ['0001-first.md']);
