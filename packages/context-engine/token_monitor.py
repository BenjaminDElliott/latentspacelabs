...
class TokenMonitor:
    ...
    def get_snapshot(self) -> TokenUsageSnapshot:
        ...
        with self._lock:
            usage_percent = (
                (self._session_tokens / self.token_budget * 100)
            ...
            return TokenUsageSnapshot(
                ...
                timestamp=time.time(),
                threshold_exceeded=usage_percent >= self.warning_threshold,
            )

    def record_turn(self, tokens: int) -> TokenMonitorEvent | None:
        ...
        with self._lock:
            ...
            usage_percent = self._session_tokens / self.token_budget * 100 if self.token_budget > 0 else 100.0
            usage_percent = min(usage_percent, 100.0)

            # Check thresholds in order
            if usage_percent >= self.critical_threshold and not self._events_fired["critical"]:
                self._events_fired["critical"] = True
                event = TokenMonitorEvent(
                    event_type="threshold_critical",
                    usage_percent=round(usage_percent, 2),
                    timestamp=time.time(),
                    message=f"Token usage at {usage_percent:.1f}% (critical threshold: {self.critical_threshold * 100:.0f}%)",
                )
                if self.on_critical:
                    self.on_critical(event)
            elif usage_percent >= self.warning_threshold and not self._events_fired["warning"]:
                self._events_fired["warning"] = True
                event = TokenMonitorEvent(
                    event_type="threshold_warning",
                    usage_percent=round(usage_percent, 2),
                    timestamp=time.time(),
                    message=f"Token usage at {usage_percent:.1f}% (warning threshold: {self.warning_threshold * 100:.0f}%)",
                )
                if self.on_warning:
                    self.on_warning(event)

            if usage_percent >= 100.0 and not self._events_fired["exceeded"]:
    ...
    def end_turn(self) -> int:
        """Signal end of a turn, move turn tokens to session total, return count."""
        with self._lock:
            self._turn_count += 1
    ...
    def is_threshold_exceeded(self) -> bool:
        ...
        return snapshot.threshold_exceeded
