...
export function buildTicketPack(input: BuildPackInput): BuildPackResult {
  ...
  const complexityLabel =
    issue.complexityTag === 'unknown' ? 'TBD' : `complexity:${issue.complexityTag}`;
  const reasoningLabel =
    issue.reasoningTag === 'unknown' ? 'TBD' : `reasoning:${issue.reasoningTag}`;
  ...
  lines.push('');
  lines.push('- [ ] All acceptance criteria from the Linear issue body are satisfied.');
  lines.push(
  ...
  lines.push('');
  lines.push('- **Files in scope (allowlist):** as defined by the Linear issue body.');
  lines.push(
