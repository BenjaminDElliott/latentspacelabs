...
import { resolveDispatcherConfig, runDispatcher } from './dispatch.js';
...
function fakeIssue(overrides: Partial<DispatchIssue> = {}): DispatchIssue {
  return {
    ...
    labels: [],
    // Ensure complexityTag and reasoningTag are always set, even when overridden.
    ...overrides,
...
function fakeLinear(
  ...
  return {
    ...
    async createRunRecord(issue) {
      ...
      return {
        id: `run-record-${issue.parentId}`,
        url: `https://linear.app/issue/${issue.parentId}/run-record`,
      };
    },
  };
}
...
function planSpawn(
  plan: SpawnPlan,
  captured: { args: ReadonlyArray<string>[]; cwd: (string | undefined)[] },
): DispatcherSpawn {
 ...
 */
function fakeWorktreeAllocator(
  repoRoot: string,
  opts: { gitLog?: GitCall[]; suffix?: () => string } = {},
): WorktreeAllocator {
...
test('LAT-140: runDispatcher emits sanitised run artefact and references it in the Linear comment', async () => {
  ...
  await withTempDir(async (dir) => {
    ...
    assert.match(r.artefact?.redaction.pre_redaction_payload_sha256 ?? '', /^[0-9a-f]{64}$/);
...
test('runDispatcher: LAT-143 exit-0 with unparseable summary is treated as no_review_artifact, not silently promoted', async () => {
  ...
  await withTempDir(async (dir) => {
    const r = await runDispatcher({
      ...
      deps: {
        linear,
        spawn: planSpawn({ stdout: 'this is not json at all', exitCode: 0 }, captured),
        worktree: fakeWorktreeAllocator(dir),
      },
    });
    assert.equal(r.outcome, 'no_review_artifact');
