...
function recordingFetch(responses: ReadonlyArray<FetchLikeResponse>): {
  fn: FetchLike;
  calls: FetchCall[];
} {
...
test('parseDispatchFields: Sequencing with blockers but no explicit status → blocked', () => {
  const desc = ['## Sequencing', '- Hard blockers: LAT-99', ''].join('\n');
  const parsed = parseDispatchFields(desc);
...
test('readIssue: issue=null → typed issue_not_found error', async () => {
  ...
  await assert.rejects(
    ...
    (err: unknown) => err instanceof LinearAdapterError && err.kind === 'issue_not_found',
  );
});
...
test('readIssue: 429 → rate_limited', async () => {
  ...
  await assert.rejects(
    ...
    (err: unknown) => err instanceof LinearAdapterError && err.kind === 'rate_limited',
  );
});
...
test('readIssue: fetch throws (network) → network_error', async () => {
  ...
  await assert.rejects(
    () => adapter.readIssue('LAT-1'),
    (err: unknown) => err instanceof LinearAdapterError && err.kind === 'network_error',
  );
});
...
test('readIssue: empty identifier → malformed_ticket', async () => {
  ...
  await assert.rejects(
    () => adapter.readIssue('   '),
    (err: unknown) => err instanceof LinearAdapterError && err.kind === 'malformed_ticket',
  );
});
...
test('readIssue: emits sanitised lifecycle events (no api key)', async () => {
  ...
  for (const e of events) {
    assert.ok(!JSON.stringify(e).includes(SYNTHETIC_KEY), 'events must not contain api key');
  }
});
...
test('postComment: commentCreate.success=false → api_error', async () => {
  ...
  await assert.rejects(
    ...
    (err: unknown) => err instanceof LinearAdapterError && err.kind === 'api_error',
  );
});
...
test('postComment: 403 during resolution → unauthorized', async () => {
  ...
  await assert.rejects(
    ...
    (err: unknown) => err instanceof LinearAdapterError && err.kind === 'unauthorized',
  );
});
