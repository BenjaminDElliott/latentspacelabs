...
function makeRequest(partial: Partial<AgentInvocationRequest> = {}): AgentInvocationRequest {
  return {
    agent_type: 'coding',
...
function makeResult(partial: Partial<AgentInvocationResult> = {}): AgentInvocationResult {
  return {
...
test('post-run gate blocked: cost_band escalated normal → elevated', () => {
  ...
  assert.ok(blocked.blockedBy.some((a: { code: string }) => a.code === 'cost_band_escalated'));
});
...
test('post-run gate blocked: spent_usd exceeds budget_cap_usd', () => {
  ...
  assert.equal(outcome.kind, 'blocked');
  const blocked = outcome as { kind: 'blocked'; blockedBy: unknown[] };
  assert.ok(blocked.blockedBy.some((a: { code: string }) => a.code === 'cost_exceeds_budget'));
});
...
test('post-run gate blocked: dry run produced a PR URL', () => {
  ...
  assert.equal(outcome.kind, 'blocked');
  const blocked = outcome as { kind: 'blocked'; blockedBy: unknown[] };
  assert.ok(blocked.blockedBy.some((a: { code: string }) => a.code === 'unexpected_pr_url'));
});
...
test('post-run gate blocked: agent force-pushed at L3', () => {
  ...
  assert.equal(outcome.kind, 'blocked');
  const blocked = outcome as { kind: 'blocked'; blockedBy: unknown[] };
  assert.ok(blocked.blockedBy.some((a: { code: string }) => a.code === 'autonomy_escalated'));
});
...
test('post-run gate blocked: missing cost band on side-effecting run', () => {
  ...
  assert.equal(outcome.kind, 'blocked');
  const blocked = outcome as { kind: 'blocked'; blockedBy: unknown[] };
  assert.ok(blocked.blockedBy.some((a: { code: string }) => a.code === 'missing_cost_band'));
});
...
test('post-run gate blocked: needs_human with error notes on side-effecting run', () => {
  ...
  assert.equal(outcome.kind, 'blocked');
  const blocked = outcome as { kind: 'blocked'; blockedBy: unknown[] };
  assert.ok(blocked.blockedBy.some((a: { code: string }) => a.code === 'result_status_mismatch'));
});
