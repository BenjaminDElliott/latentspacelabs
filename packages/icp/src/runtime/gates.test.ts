...
function makeInput(partial: Partial<InvocationGateInput> = {}): InvocationGateInput {
  return {
    agent_type: 'coding',
...
test('gate blocked: repo_forbidden', () => {
  ...
  assert.equal(outcome.kind, 'blocked');
  const blocked = outcome as { kind: 'blocked'; blockedBy: unknown[] };
  assert.ok(blocked.blockedBy.some((a: { code: string }) => a.code === 'repo_forbidden'));
});
...
test('gate blocked: branch_target_forbidden', () => {
  ...
  assert.equal(outcome.kind, 'blocked');
  const blocked = outcome as { kind: 'blocked'; blockedBy: unknown[] };
  assert.ok(blocked.blockedBy.some((a: { code: string }) => a.code === 'branch_target_forbidden'));
});
...
test('gate blocked: autonomy_exceeds_cap', () => {
  ...
  assert.equal(outcome.kind, 'blocked');
  const blocked = outcome as { kind: 'blocked'; blockedBy: unknown[] };
  assert.ok(blocked.blockedBy.some((a: { code: string }) => a.code === 'autonomy_exceeds_cap'));
});
...
test('gate blocked: budget_cap_missing', () => {
  ...
  assert.equal(outcome.kind, 'blocked');
  const blocked = outcome as { kind: 'blocked'; blockedBy: unknown[] };
  assert.ok(blocked.blockedBy.some((a: { code: string }) => a.code === 'budget_cap_missing'));
});
...
test('gate blocked: cost_band_runaway_risk', () => {
  ...
  assert.equal(outcome.kind, 'blocked');
  const blocked = outcome as { kind: 'blocked'; blockedBy: unknown[] };
  assert.ok(blocked.blockedBy.some((a: { code: string }) => a.code === 'cost_band_runaway_risk'));
});
...
test('gate blocked: forbidden_ticket_context (guardrail match)', () => {
  ...
  assert.equal(outcome.kind, 'blocked');
  const blocked = outcome as { kind: 'blocked'; blockedBy: unknown[] };
  assert.ok(blocked.blockedBy.some((a: { code: string }) => a.code === 'forbidden_ticket_context'));
});
...
test('gate: dry_run still checks agent_type, repo, branch_target, guardrails', () => {
  ...
  assert.ok(blocked.blockedBy.some((a: { code: string }) => a.code === 'agent_type_forbidden'));
  assert.ok(blocked.blockedBy.some((a: { code: string }) => a.code === 'repo_forbidden'));
  assert.ok(blocked.blockedBy.some((a: { code: string }) => a.code === 'branch_target_forbidden'));
  assert.ok(blocked.blockedBy.some((a: { code: string }) => a.code === 'forbidden_ticket_context'));
});
