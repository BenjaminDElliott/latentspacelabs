...
test('retro: clean runs (`none`) are never candidates', () => {
  const runs = [run('r1', 'LAT-1', 'none'), run('r2', 'LAT-2', 'none'), run('r3', 'LAT-3', 'none')];
...
test('retro: suggestion vocabulary matches ADR-0010 promotion paths', () => {
  const runs = [run('r1', 'LAT-1', 'cost_runaway_risk'), run('r2', 'LAT-2', 'cost_runaway_risk')];
  const res = aggregateRunsForRetro(runs, { threshold: 2 });
  assert.equal(res.candidates.length, 1);
...
test('retro: invalid threshold falls back to 2', () => {
  const runs = [run('r1', 'LAT-1', 'missing_approval'), run('r2', 'LAT-2', 'missing_approval')];
