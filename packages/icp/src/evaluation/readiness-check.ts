 ...
 */
export function evaluateReadiness(input: ReadinessTicketInput): ReadinessReport {
  const reasons: ReadinessReason[] = [];
  const state: { verdict: ReadinessVerdict } = { verdict: 'ready' };

  const push = (level: ReadinessVerdict, category: FailureCategory, message: string): void => {
  ...
  const refusalMarkdown = state.verdict === 'refuse' ? renderRefusalMarkdown(reasons) : null;
