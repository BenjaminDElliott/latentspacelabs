...
import { dispatchTicketSkill, type DispatchTicketOutputs } from './dispatch-ticket.js';

const HERE = fileURLToPath(new URL('.', import.meta.url));
const REPO_ROOT = resolve(HERE, '..', '..', '..', '..');

function readyIssue(overrides: Partial<LinearIssueSnapshot> = {}): LinearIssueSnapshot {
  return {
    id: 'LAT-999',
    status: 'todo',
    sequencing: {
...
async function buildHarness(issues: Record<string, LinearIssueSnapshot>): Promise<Harness> {
...
test('LAT-66: dispatch-ticket surfaces unknown + typed reason when provider withholds cost data', async () => {
  ...
  const resolved: ResolvedTools = {
    ...
    agents: createStubAgentAdapter({
      responses: {
        [issue.id]: {
          cost_band: 'unknown',
          cost_band_unavailable_reason: 'command provider did not emit spend metrics',
        },
      },
    }),
    runRecorder: createRunRecorder(),
  ...
  assert.ok(
    outputs.cost_band_unavailable_reason && outputs.cost_band_unavailable_reason.length > 0,
