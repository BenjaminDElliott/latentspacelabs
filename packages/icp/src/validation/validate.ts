...
export class VersionMismatchError extends Error {
  ...
  constructor({
    ...
    super(`Schema version mismatch for ${schemaName}: expected ${expected}, got ${actual}`);
  }
}
...
function requireString(
  ...
  if (typeof value !== 'string' || value.length === 0) {
    addError(
      errors,
      fieldPath,
      `must be a non-empty string, got ${value === undefined ? 'undefined' : typeof value}`,
    );
    return null;
  }
  return value;
...
function requireNonNegative(
  ...
  if (typeof value !== 'number' || value < 0) {
    addError(
      errors,
      fieldPath,
      `must be a non-negative number, got ${value === undefined ? 'undefined' : typeof value}`,
    );
    return null;
  }
  return value;
...
function requirePositive(
  ...
  if (typeof value !== 'number' || value <= 0) {
    addError(
      errors,
      fieldPath,
      `must be a positive number, got ${value === undefined ? 'undefined' : typeof value}`,
    );
    return null;
  }
  return value;
...
function requireEnum(
  ...
  if (!allowed.includes(value as string)) {
    addError(
      errors,
      fieldPath,
      `must be one of [${allowed.join(', ')}], got "${value === undefined ? 'undefined' : String(value)}"`,
    );
    return null;
  }
...
function requireNullableString(
  ...
  if (typeof value !== 'string' || value.length === 0) {
    addError(
      errors,
      fieldPath,
      `must be a non-empty string or null/undefined, got ${typeof value}`,
    );
    return null;
  }
  return value;
...
function requireStringArray(
  ...
  if (!Array.isArray(value)) {
    addError(
      errors,
      fieldPath,
      `must be an array of strings, got ${value === undefined ? 'undefined' : typeof value}`,
    );
    return null;
  }
...
function requireNonEmptyStringArray(
  ...
  if (!Array.isArray(value) || value.length === 0) {
    addError(
      errors,
      fieldPath,
      `must be a non-empty array of strings, got ${value === undefined ? 'undefined' : `array of ${value.length}`}`,
    );
    return null;
  }
...
function requireUrl(errors: ValidationError[], value: unknown, fieldPath: string): string | null {
 ...
 */
export function validateRunReport(report: unknown): ValidationResult<RunReport> {
  ...
  // agent_type
  requireEnum(errors, r.agent_type, 'agent_type', [
    'coding',
    'qa',
    'review',
    'sre',
  ...
  // status
  requireEnum(errors, r.status, 'status', [
    'started',
    'succeeded',
    'failed',
    'cancelled',
  ...
  // triggered_by
  requireEnum(errors, r.triggered_by, 'triggered_by', [
    'user',
    'linear_status',
    'schedule',
    'webhook',
  ...
  // autonomy_level
  requireEnum(errors, r.autonomy_level, 'autonomy_level', [
    'L1-read-only',
    'L2-propose',
    'L3-with-approval',
    'L4-autonomous',
  ...
  if (cost === null || cost === undefined || typeof cost !== 'object') {
    ...
    requireEnum(errors, c.band, 'cost.band', ['normal', 'elevated', 'runaway_risk', 'unknown']);
 ...
 */
export function validateRunArtefact(artefact: unknown): ValidationResult<RunArtefact> {
  const errors: ValidationError[] = [];
  ...
  if (a.classifier !== null && a.classifier !== undefined) {
    if (typeof a.classifier !== 'object') {
      ...
      requireEnum(errors, cl.risk_class, 'classifier.risk_class', [
        'low',
        'medium',
        'high',
        'unknown',
      ...
      requireEnum(errors, cl.required_human_approval, 'classifier.required_human_approval', [
        true,
        false,
      ]);
  ...
  // outcome
  requireEnum(errors, a.outcome, 'outcome', [
    'succeeded',
    'ready_for_review',
    'checks_failed',
    'refused',
  ...
  if (!Array.isArray(checks)) {
    ...
    for (let i = 0; i < checks.length; i++) {
      ...
      requireEnum(errors, ck.outcome, `checks[${i}].outcome`, [
        'passed',
        'failed',
        'skipped',
        'manual',
  ...
  if (!Array.isArray(accCov)) {
    ...
    for (let i = 0; i < accCov.length; i++) {
      ...
      requireEnum(errors, ac.status, `acceptance_criteria_coverage[${i}].status`, [
        'covered',
        'partial',
        'uncovered',
        'unknown',
  ...
  requireEnum(errors, a.training_eligibility, 'training_eligibility', [
    'eligible',
    'ineligible',
    'needs_human_decision',
  ]);
  requireEnum(errors, a.quality_label, 'quality_label', [
    'ready_for_review',
    'needs_review',
    'low_quality',
    'unknown',
...
export function validateAgentInvocationRequest(
  ...
  // cost_band_observed
  requireEnum(errors, r.cost_band_observed, 'cost_band_observed', [
    'normal',
    'elevated',
    'runaway_risk',
    'unknown',
  ...
  if (
    typeof r.skill_name_and_version !== 'undefined' &&
    typeof r.skill_name_and_version !== 'string'
  ) {
...
export function validateAgentInvocationResult(
  ...
  // exit_signal
  requireEnum(errors, r.exit_signal, 'exit_signal', [
    'succeeded',
    'failed',
    'cancelled',
    'needs_human',
 ...
 */
export function validatePolicyEvaluation(eval_: unknown): ValidationResult<PolicyEvaluation> {
  const errors: ValidationError[] = [];
...
export function validateOutput(output: unknown): ValidationResult {
  ...
  if (typeof obj.exit_signal === 'string' && typeof obj.cost_band === 'string') {
  ...
  if (typeof obj.agent_type === 'string' && typeof obj.approve === 'boolean') {
