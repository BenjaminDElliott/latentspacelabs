...
export function renderCockpitSummary(state: CockpitState, options: SummaryOptions = {}): string {
  ...
  lines.push('');
  lines.push(
    `_Rendered at ${state.rendered_at}. Source: repo-committed runs/ tree (+ Linear/GitHub where supplied). PRD: docs/prds/LAT-28-icp-observability-cockpit.md._`,
  );
  lines.push('');
...
function renderFailedGroup(lines: string[], g: FailedRunsGroup, runsHref: string): void {
  const refs = g.run_ids.map((id) => `[\`${id}\`](${runsHref}${id}.md)`).join(', ');
...
function renderLearning(lines: string[], c: LearningCandidate, runsHref: string): void {
  const refs =
    c.run_ids.length > 0
      ? ' · runs: ' + c.run_ids.map((id) => `[\`${id}\`](${runsHref}${id}.md)`).join(', ')
      : '';
  const span = c.span.start || c.span.end ? ` · span: ${c.span.start} → ${c.span.end}` : '';
  lines.push(
    `- **${c.kind}** \`${c.cluster_key}\` · count=${c.count}${span}${refs} · ${c.retro_intake_hint}`,
  );
}

function formatFindings(f: Readonly<Record<string, number>>): string {
