...
export function buildCockpitState(inputs: CockpitInputs): CockpitState {
  ...
  const failedWindowDays = inputs.failed_window_days ?? DEFAULT_FAILED_WINDOW_DAYS;
  const activeWindowHours = inputs.active_runs_window_hours ?? DEFAULT_ACTIVE_WINDOW_HOURS;
  const recentLimit = inputs.recent_completions_limit ?? DEFAULT_RECENT_LIMIT;

  const runs = [...inputs.runs];
  const linearIssues = new Map((inputs.linear_issues ?? []).map((i) => [i.id, i]));
  ...
  const learning_candidates = projectLearningCandidates(runs, now, failedWindowDays, qaReports);
...
function projectCostRiskFlags(
  ...
  rows.sort((a, b) => b.severity_rank - a.severity_rank || a.run_id.localeCompare(b.run_id));
  return rows;
}
...
function projectPRReviewQueue(
  ...
  for (const pr of prs ?? []) {
    ...
    const run = runs.find((r) => r.linear_issue_id === lat || r.correlation.pr_url === pr.url);
...
function projectLearningCandidates(
  ...
  for (const [klass, prNums] of byFinding) {
    ...
    out.push({
      ...
      span: { start: '', end: '' },
      run_ids: [],
      retro_intake_hint: 'candidate for ADR-0010 retro intake: recurring PR-review finding class',
    });
  }
