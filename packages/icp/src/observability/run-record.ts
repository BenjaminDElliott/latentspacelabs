 ...
 */
export function buildRunRecordTitle(
...
export function buildRunRecordDescription(artefact: RunArtefact): string {
  ...
  lines.push(`| --- | --- |`);
  lines.push(
    `| **Run ID** | \`${artefact.invocation_id}\` |`,
  );
  lines.push(`| **Outcome** | ${artefact.outcome} |`);
  lines.push(`| **Status** | ${artefact.outcome === "ready_for_review" ? "ready" : "closed"} |`);
  lines.push(
    `| **Started** | ${artefact.started_at} |`,
  );
  lines.push(`| **Ended** | ${artefact.ended_at} |`);
  lines.push(
    `| **Duration** | ${formatDuration(artefact.duration_ms)} |`,
  );
  lines.push(
    `| **Ticket** | ${artefact.ticket_id ?? "n/a"} |`,
  );
  lines.push(
    `| **Branch** | ${artefact.branch ?? "n/a"} |`,
  );
  lines.push(
    `| **PR** | ${artefact.pr_url ?? "n/a"} |`,
  );
  lines.push(
    `| **Provider** | ${artefact.provider ?? "n/a"} |`,
  );
  lines.push(
    `| **Cost class** | ${artefact.cost_class} |`,
  );
  lines.push(
    `| **Risk class** | ${artefact.risk_level} |`,
  );
  lines.push("");
        ...
        check.outcome === "passed"
          ...
          : check.outcome === "failed"
            ...
            : check.outcome === "skipped"
              ...
              : "🤷";
      lines.push(
        `- ${icon} \`${check.name}\` — ${check.outcome} (${check.durationMs}ms)`,
      );
    }
    lines.push("");
  }
  ...
  if (
    artefact.acceptance_criteria_coverage &&
    artefact.acceptance_criteria_coverage.length > 0
  ) {
  ...
  lines.push(
    `- **Patterns:** ${artefact.redaction.applied_patterns.join(", ")}`,
  );
  lines.push(
  ...
  lines.push(
    `- **Artefact class:** ${artefact.artefact_class}`,
  );
  lines.push(
    `- **Training eligibility:** ${artefact.training_eligibility}`,
  );
