...
describe('validateTicketPack', () => {
  ...
  it('rejects branch that does not match the Linear ID', () => {
    const result = validateTicketPack(packBuilder({ branch: 'lat-99-other-thing' }), '/tmp/p.md');
    assert.equal(result.ok, false);
    assert.ok(result.findings.some((f) => f.code === 'branch_ticket_mismatch'));
  });

  it('rejects PR base that is not main', () => {
    const result = validateTicketPack(packBuilder({ prBase: 'develop' }), '/tmp/p.md');
    assert.equal(result.ok, false);
  ...
  it('rejects empty allowlist', () => {
    const result = validateTicketPack(packBuilder({ filesInScope: '' }), '/tmp/p.md');
    assert.equal(result.ok, false);
    assert.ok(
      result.findings.some((f) => f.code === 'files_in_scope_empty' || f.code === 'parse_error'),
    );
  });
  ...
  it('rejects allowlist that touches a forbidden root', () => {
    ...
    assert.equal(result.ok, false);
    assert.ok(result.findings.some((f) => f.code === 'allowlist_in_forbidden_root'));
  });
  ...
  it('rejects empty acceptance criteria', () => {
    const result = validateTicketPack(packBuilder({ acceptance: '' }), '/tmp/p.md');
    assert.equal(result.ok, false);
    assert.ok(result.findings.some((f) => f.code === 'acceptance_criteria_empty'));
  });
});
