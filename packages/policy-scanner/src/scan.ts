...
export const DEFAULT_CONFIG: PolicyConfig = {
  ...
  pythonToolingDirectories: ['scripts', 'tools', 'bin', '.github/scripts', '.github/workflows'],
};
...
function resolveConfig(partial: Partial<PolicyConfig> | undefined): ResolvedConfig {
  return {
    ...
    ignoreDirectories: [...DEFAULT_CONFIG.ignoreDirectories, ...(partial?.ignoreDirectories ?? [])],
    pythonToolingDirectories:
      partial?.pythonToolingDirectories ?? DEFAULT_CONFIG.pythonToolingDirectories,
  };
}

export async function loadConfigFromFile(root: string): Promise<Partial<PolicyConfig> | undefined> {
