...
test('duplicate prd_id fails', async () => {
  ...
  try {
    ...
    const result = await validatePrdDirectory(dir);
    const dupErr = result.errors.find((e) => e.message.includes('duplicate frontmatter prd_id'));
...
test('feature PRD with empty derived_from fails', async () => {
  ...
  try {
    ...
    const result = await validatePrdDirectory(dir);
    const err = result.errors.find((e) => e.message.includes('feature PRD must set derived_from'));
...
test('feature PRD derived_from pointing at unknown stem fails', async () => {
  ...
  try {
    ...
    const result = await validatePrdDirectory(dir);
    const err = result.errors.find((e) => e.message.includes("derived_from 'root-nonexistent'"));
...
test('feature PRD derived_from pointing at a feature PRD fails', async () => {
  ...
  try {
    ...
    const result = await validatePrdDirectory(dir);
    const err = result.errors.find((e) => e.message.includes('is not a root PRD'));
...
test('feature PRD filename LAT-NN not in related_linear fails', async () => {
  ...
  try {
    ...
    const result = await validatePrdDirectory(dir);
    const err = result.errors.find((e) => e.message.includes('does not appear in related_linear'));
