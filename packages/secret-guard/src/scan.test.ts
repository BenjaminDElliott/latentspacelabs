...
test('scanText accepts placeholder ${VAR} in JSON-quoted form (LAT-118)', () => {
  const findings = scanText({
    file: '.mcp.json',
    text: [`{`, `  "env": {`, `    "AUTH_TOKEN": "\${AUTH_TOKEN}"`, `  }`, `}`].join('\n'),
  });
  assert.deepEqual(findings, [], formatResult({ findings, filesScanned: 0 }));
});

test('formatResult summarises clean and dirty results', () => {
  assert.match(formatResult({ findings: [], filesScanned: 3 }), /no findings/);
  const dirty = formatResult({
    findings: [{ rule: 'x', severity: 'error', file: 'a', line: 2, message: 'm' }],
    filesScanned: 1,
  });
